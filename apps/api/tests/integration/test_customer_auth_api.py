...
def test_register_creates_session_and_me(client: TestClient):
...
def test_register_rejects_duplicate_email(client: TestClient):
    ...
    duplicate = register(client, name="Someone Else")
...
def test_google_not_configured_is_rejected(client: TestClient):
...
def test_login_is_rate_limited_per_account(client: TestClient):
    register(client)
    client.cookies.clear()
