...
def _header_value(headers: dict[str, str], name: str) -> str | None:
    """Case-insensitive lookup into a plain header dict.

    The function is typed as `dict[str, str]` (not Starlette's `Headers`) so it
    stays trivially testable without constructing a real `Request` — but that
    means it does not inherit `Headers`' case-insensitivity for free, so this
    recovers it explicitly. PayPal's docs render the transmission headers in
    upper case; the ASGI server that hands them to us normalizes to lower case;
    neither is assumed.
    """
    target = name.lower()
...
async def verify_paypal_webhook_signature(
    settings: Settings, *, headers: dict[str, str], raw_body: bytes, webhook_event: dict[str, Any]
) -> bool:
    """Verify a PayPal webhook delivery via PayPal's own verification API.

    Unlike Razorpay's webhook — a local HMAC over the raw body we can check
    entirely offline — PayPal signs webhook deliveries with an asymmetric key
    and expects the receiver to call back into its API with the transmission
    headers plus the parsed event body; PayPal alone can confirm the signature
    was produced with its private key. `raw_body` is accepted for symmetry
    with the Razorpay verifier and so a future switch to local verification
    (fetching `cert_url` and checking the signature offline) does not change
    this function's call sites, but it is unused today.

    A verification failure here must only ever mean "reject the webhook" — this
    runs in a server-to-server context with no customer watching, so unlike
    the checkout-flow PayPal functions above it never raises `PaymentError`.
    Any required header missing, or no webhook id configured, is treated as
    unverifiable without spending a round trip on PayPal's API.
    """
    del raw_body  # kept for interface symmetry with the local Razorpay verifier
    ...
    except (HttpError, PaymentError) as exc:
        # `_paypal_access_token` raises `PaymentError` on a token failure;
        # `post_json_async` raises `HttpError` on a transport/HTTP failure.
        # Both collapse to "could not verify" — never a customer-facing error.
        log_event("error", "paypal_webhook_verify_failed", error_type=type(exc).__name__)
        return False
