...
class ChatRoomDO(DurableObject):
    ...
    async def _handle_message(self, ws: Any, message: Any) -> None:
        ...
        # getWebSockets, not get_websockets -- confirmed live via wrangler
        # tail ("AttributeError: get_websockets. Did you mean:
        # 'getWebSockets'?"). Same raw-JS-camelCase-on-the-binding pattern as
        # acceptWebSocket above; unlike storage.put/get (real Python SDK
        # wrapper methods), this one is not pythonized.
