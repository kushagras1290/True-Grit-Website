...
const categoryRows = [
  [
    "wheat-flour",
    "Wheat Flour",
    "Traditional single-variety whole-wheat flours milled in small batches for rotis, parathas and everyday cooking.",
  ],
  [
    "cold-pressed-oils",
    "Cold-Pressed Oils",
    "Traditional mustard, flaxseed and sesame oils pressed in small batches for distinctive flavour and everyday Indian cooking.",
  ],
  [
    "seeds",
    "Seeds",
    "Plain and carefully roasted flax and sesame seeds for chutneys, laddoos, toppings and pantry staples.",
  ],
  [
    "black-gram",
    "Black Gram",
    "Whole and split black gram, along with urad sattu and besan, for dals, batters, breads and drinks.",
  ],
  [
    "red-lentils",
    "Red Lentils",
    "Whole and split masoor selected for quick, dependable dals, curries and chillas.",
  ],
  [
    "daliya",
    "Daliya",
    "Traditional broken wheat in Kathiya, Banshi and Paigambari varieties for savoury breakfasts, khichdi and desserts.",
  ],
  [
    "semolina",
    "Semolina",
    "Small-batch wheat semolina in three traditional varieties for upma, halwa, cheela and baking.",
  ],
  [
    "whole-wheat-pasta",
    "Whole-Wheat Pasta",
    "Rustic pasta made from Kathiya, Banshi and Paigambari wheat for sauces, vegetables and Indian-style pasta.",
  ],
  [
    "whole-wheat-vermicelli",
    "Whole-Wheat Vermicelli",
    "Traditional-wheat vermicelli for savoury upma, kheer, seviyan and quick one-pot meals.",
  ],
  [
    "white-field-peas",
    "White Field Peas",
    "Whole white field peas for curries, chaat, ragda and hearty everyday meals.",
  ],
];
...
const imageFor = (section, title) => {
  ...
  if (/urad|black gram|sattu|besan/.test(value))
    return "/catalogue/categories/banners/black-gram.webp";
  if (/masoor|red lentil/.test(value)) return "/catalogue/categories/banners/red-lentils.webp";
  if (/white pea|field pea/.test(value))
    return "/catalogue/categories/banners/white-field-peas.webp";
  ...
  if (/vermicelli|seviyan/.test(value))
    return "/catalogue/categories/banners/whole-wheat-vermicelli.webp";
  if (/pasta/.test(value)) return "/catalogue/categories/banners/whole-wheat-pasta.webp";
  if (/wheat|flour|atta|roti|chapati|paratha/.test(value))
    return "/catalogue/categories/banners/wheat-flour.webp";
...
const familyFor = (section, title) => {
  const value = `${section} ${title}`.toLowerCase();
  if (/mustard|oil/.test(value))
    return {
      ingredient: "cold-pressed oil",
      use: "tadka, pickles and regionally appropriate cooking",
      storage: "a tightly closed bottle in a cool, dark cupboard",
      cue: "a clean aroma without stale or paint-like notes",
    };
  if (/flax|linseed/.test(value))
    return {
      ingredient: "flax seed",
      use: "chutneys, laddoos, toppings and small-batch oil",
      storage: "an airtight jar away from heat; refrigerate ground seed",
      cue: "a mild nutty aroma without bitterness",
    };
  if (/sesame|til/.test(value))
    return {
      ingredient: "sesame",
      use: "chutneys, tempering, sweets and seed pastes",
      storage: "an airtight jar away from heat and moisture",
      cue: "a fresh nutty aroma and even colour",
    };
  if (/urad|black gram|sattu|besan/.test(value))
    return {
      ingredient: "black gram",
      use: "dal, fermented batter, chilla, sattu drinks and breads",
      storage: "a dry airtight container checked periodically for moisture or insects",
      cue: "clean, evenly coloured grain with no musty smell",
    };
  if (/masoor|red lentil/.test(value))
    return {
      ingredient: "red lentil",
      use: "quick dals, curries, soups and chillas",
      storage: "a dry airtight container away from sunlight",
      cue: "clean lentils with even colour and no powdery residue",
    };
  if (/white pea|field pea/.test(value))
    return {
      ingredient: "white field pea",
      use: "ragda, chaat, curries and hearty salads",
      storage: "a dry airtight container, ideally in manageable monthly quantities",
      cue: "whole, clean peas that hydrate evenly",
    };
  if (/daliya/.test(value))
    return {
      ingredient: "daliya",
      use: "savoury breakfasts, one-pot khichdi and milk-based desserts",
      storage: "an airtight container in a cool, dry cupboard",
      cue: "separate grains with a clean wheat aroma",
    };
  if (/semolina|suji|upma|halwa/.test(value))
    return {
      ingredient: "semolina",
      use: "upma, halwa, cheela, coatings and baking",
      storage: "an airtight jar away from warmth and humidity",
      cue: "an even grind and clean cereal aroma",
    };
  if (/vermicelli|seviyan/.test(value))
    return {
      ingredient: "whole-wheat vermicelli",
      use: "upma, kheer, seviyan and quick one-pot meals",
      storage: "a rigid airtight container to prevent moisture and breakage",
      cue: "dry, separate strands with no stale aroma",
    };
  if (/pasta/.test(value))
    return {
      ingredient: "whole-wheat pasta",
      use: "vegetable sauces, light oil dressings and Indian masala preparations",
      storage: "a dry airtight container after opening",
      cue: "an even wheat colour and firm dry shape",
    };
  return {
    ingredient: "traditional wheat",
    use: "rotis, parathas, daliya and other everyday preparations",
    storage: "an airtight container in a cool, dry place, bought in quantities used while fresh",
    cue: "a fresh grain aroma, even texture and no rancid note",
  };
};
...
const recipeTitle = (topic) => {
  ...
  return /\brecipe\b|how to make perfect suji halwa|indian masala pasta|garlic sesame whole-wheat pasta|mustard-oil vegetable pasta|how to make vermicelli upma|traditional seviyan|aloo fry in mustard oil|bengali-style vegetables in mustard oil|mustard oil pickle|traditional achaar/i.test(
    topic.title,
  );
};
...
const articleBlocks = (topic) => {
  ...
  return {
    blocks: [
      {
        id: `blk_article_${topic.number}_guide`,
        type: "rich_text",
        version: 1,
        enabled: true,
        props: { heading: "A practical guide", paragraphs },
      },
      {
        id: `blk_article_${topic.number}_checklist`,
        type: "faq",
        version: 1,
        enabled: true,
        props: {
          heading: "Questions readers often ask",
          items: [
            {
              question: "What should I check first when buying?",
              answer: `Check the product form, pack date, ingredient list and ${family.cue}. Choose a quantity your household can finish while it is still fresh.`,
            },
            {
              question: "Does small-batch food always look identical?",
              answer:
                "No. Natural crops can vary slightly in colour, size and cooking time. Moisture, pests, mould or rancid odours are not acceptable variation.",
            },
            {
              question: "How should I test a new variety?",
              answer:
                "Cook a small amount in a familiar recipe, keep the other variables steady, and record water, time, texture and flavour before changing the method.",
            },
            {
              question: "What is the safest storage routine?",
              answer: `Use ${family.storage}, a dry utensil, an opening-date label and first-in, first-out rotation. Inspect the ingredient before every use.`,
            },
          ],
        },
      },
...
const recipeProfile = (title) => {
  ...
  let base;
  if (/daliya/.test(lower))
    base = {
      main: "daliya",
      product: "daliya-kathiya-wheat",
      ratio: "1 cup daliya to 2½ cups liquid",
      prep: 15,
      cook: 30,
    };
  else if (/vermicelli|seviyan/.test(lower))
    base = {
      main: "whole-wheat vermicelli",
      product: "vermicelli-kathiya-wheat",
      ratio: "1 cup vermicelli to 1¾ cups liquid",
      prep: 15,
      cook: 20,
    };
  else if (/pasta/.test(lower))
    base = {
      main: "whole-wheat pasta",
      product: "pasta-kathiya-wheat",
      ratio: "2 litres well-salted water per 200 g pasta",
      prep: 15,
      cook: 25,
    };
  else if (/suji|semolina|upma|halwa/.test(lower))
    base = {
      main: "semolina",
      product: "semolina-kathiya-wheat",
      ratio: "1 cup semolina to 2½ cups liquid",
      prep: 10,
      cook: 25,
    };
  else if (/white pea/.test(lower))
    base = {
      main: "white field peas",
      product: "field-pea-white",
      ratio: "1 cup soaked peas to 2½ cups water",
      prep: 20,
      cook: 45,
    };
  else if (/masoor/.test(lower))
    base = {
      main: "red lentils",
      product: "red-lentils-broken",
      ratio: "1 cup lentils to 2½ cups water",
      prep: 15,
      cook: 30,
    };
  else if (/sesame|til/.test(lower))
    base = {
      main: "sesame seeds",
      product: "sesame-seed",
      ratio: "toast gently and add to taste",
      prep: 20,
      cook: 25,
    };
  else if (/flax/.test(lower))
    base = {
      main: "flax seeds",
      product: "plain-flax-seed",
      ratio: "toast gently before grinding",
      prep: 20,
      cook: 25,
    };
  else
    base = {
      main: "black gram",
      product: /sattu/.test(lower) ? "black-gram-sattu" : "black-gram-broken-dal",
      ratio: "1 cup soaked gram to 3 cups water",
      prep: 20,
      cook: 40,
    };
  ...
  const ingredients = drink
    ...
    : sweet
      ...
      : [
          [base.main, "1 cup"],
          ["cold-pressed mustard or sesame oil", "1½ tablespoons"],
          ["onion", "1 medium, finely chopped"],
          ["tomato", "1 medium, chopped"],
          ["mixed seasonal vegetables", "1½ cups"],
          ["ginger", "1 teaspoon, grated"],
          ["cumin or mustard seeds", "1 teaspoon"],
          ["turmeric", "¼ teaspoon"],
          ["coriander powder", "1 teaspoon"],
          ["water", base.ratio.split(" to ").at(-1)],
          ["salt", "to taste"],
          ["lemon and coriander", "to finish"],
        ];
  const steps = drink
    ? [
        "Whisk the main ingredient with one cup of water until completely smooth and lump-free.",
        "Add the remaining water gradually while whisking so no dry pockets remain.",
        "Stir in the roasted cumin, lemon juice and black salt until evenly distributed.",
        "Taste carefully and choose either a lightly sweet or distinctly savoury balance.",
        "Chill the mixture for 15 minutes so the spice and citrus flavours settle.",
        "Stir thoroughly once more because natural grain solids may settle at the bottom.",
        "Pour over fresh ice and finish each glass with torn mint leaves.",
        "Serve immediately; refrigerate any leftovers promptly and use them within the same day.",
      ]
    : [
        `Measure every ingredient and inspect the ${base.main} before starting.`,
        sweet
          ? `Dry-roast the ${base.main} over low heat until aromatic, without darkening it.`
          : `Rinse or wipe the ${base.main} as appropriate, then drain it well.`,
        sweet
          ? "Warm the liquid separately so it does not shock the roasted grain."
          : "Heat the oil and bloom the whole spices until aromatic, not burnt.",
        sweet
          ? "Add ghee and stir until every grain is coated."
          : "Cook onion, ginger and tomato until the raw aroma disappears.",
        sweet
          ? "Pour in the hot liquid gradually while stirring to prevent lumps."
          : "Add vegetables and ground spices; cook for three to four minutes.",
        `Add the ${base.main} and measured liquid, then stir once to distribute it evenly.`,
        fry
          ? "Shape the mixture with damp hands and fry in moderately hot oil until the centre is cooked and the outside crisp."
          : "Cook gently, checking texture before adding any extra water a tablespoon at a time.",
        sweet
          ? "Add the sweetener only after the grain is tender, then cook until glossy."
          : "Rest covered for five minutes so moisture redistributes.",
...
const recipeBlocks = (topic, profile) => ({
  ...
  blocks: [
    {
      id: `blk_recipe_${topic.number}_intro`,
      type: "rich_text",
      version: 1,
      enabled: true,
      props: {
        heading: "Before you cook",
        paragraphs: [
    ...
    {
      id: `blk_recipe_${topic.number}_notes`,
      type: "rich_text",
      version: 1,
      enabled: true,
      props: {
        heading: "Make it work in your kitchen",
        paragraphs: [
    ...
    {
      id: `blk_recipe_${topic.number}_faq`,
      type: "faq",
      version: 1,
      enabled: true,
      props: {
        heading: "Recipe questions",
        items: [
          {
            question: "Can I change the wheat or pulse variety?",
            answer:
              "Yes. Keep the first batch small and expect a modest change in water absorption, cooking time and flavour.",
          },
          {
            question: "How do I avoid a mushy result?",
            answer:
              "Measure liquid, cook gently, stop stirring once the mixture is evenly distributed, and rest it covered before deciding it needs more cooking.",
          },
          {
            question: "Can I reduce the oil or ghee?",
            answer:
              "Yes, but use a heavy pan and add a spoonful of water when aromatics begin to catch. The flavour and mouthfeel will be lighter.",
          },
          {
            question: "How should leftovers be stored?",
            answer:
              "Cool promptly, refrigerate in a covered container within two hours, use within two days, and reheat until steaming hot throughout.",
          },
        ],
      },
    },
...
products.forEach(([slug, name, categorySlug, variants], index) => {
    ...
    blocks: [
      {
        ...
        props: {
          ...
          paragraphs: [
            description,
            "Because this is an agricultural, small-batch product, natural differences in colour, aroma and cooking time may occur. Store it sealed, dry and away from direct heat or sunlight.",
          ],
        },
      },
      {
        id: `blk_product_${n}_faq`,
        type: "faq",
        version: 1,
        enabled: true,
        props: {
          ...
          items: [
            {
              ...
              answer:
                "Transfer to a clean airtight container, use a dry spoon and keep away from heat, sunlight and moisture.",
            },
            {
              question: "Why can batches vary slightly?",
              answer:
                "Crop season and traditional small-batch processing can produce modest natural differences without changing the product’s identity.",
            },
          ],
        },
      },
  ...
  lines.push(
    `INSERT INTO products (id,internal_name,name,slug,product_type,farm_id,status,short_description,published_version_id,seo_title,seo_description,indexing_policy,created_at,created_by,updated_at,updated_by,archived_at,image_url,image_alt,release_scope,return_eligible,accepts_orders,payments_override,harvest_note,growing_method,storage_guidance) VALUES (${sql(productId)},${sql(`Live catalogue: ${name}`)},${sql(name)},${sql(slug)},'pantry','farm_vikas','published',${sql(description)},${sql(versionId)},${sql(`${name} — Buy Online | True Grit`)},${sql(description)},'index',${sql(now)},${sql(actor)},${sql(now)},${sql(actor)},NULL,NULL,NULL,'global',1,1,'inherit','Prepared to order in small batches.','Organic cultivation and traditional processing.','Keep sealed in a cool, dry place away from direct sunlight.');`,
  );
  lines.push(
    `INSERT INTO product_versions (id,product_id,version_number,content_json,change_summary,workflow_state,created_at,created_by,approved_at,approved_by,published_at) VALUES (${sql(versionId)},${sql(productId)},1,${jsonSql(content)},'Initial live catalogue edition','published',${sql(now)},${sql(actor)},${sql(now)},${sql(actor)},${sql(now)});`,
  );
  lines.push(
    `INSERT INTO product_categories (product_id,category_id,is_primary,sort_order,assigned_at,assigned_by) SELECT ${sql(productId)},id,1,${index + 1},${sql(now)},${sql(actor)} FROM categories WHERE slug=${sql(categorySlug)};`,
  );
  lines.push(
    `INSERT INTO search_products (product_id,name,slug,brand_name,farm_name,category_names,keywords,short_description) VALUES (${sql(productId)},${sql(name)},${sql(slug)},'True Grit','Vikas Farms',${sql(categoryRows.find(([category]) => category === categorySlug)?.[1] ?? "")},${sql(`${name} organic traditional pantry`)},${sql(description)});`,
  );
  variants.forEach(([label, weightValue, weightUnit, priceRupees], variantIndex) => {
    ...
    const sku = `TG-${n}-${String(label)
      .replace(/[^A-Z0-9]+/g, "-")
      .replace(/^-|-$/g, "")}`;
    lines.push(
      `INSERT INTO product_variants (id,product_id,sku,name,option_values_json,weight_value,weight_unit,package_description,status,sort_order,created_at,updated_at) VALUES (${sql(variantId)},${sql(productId)},${sql(sku)},${sql(label)},${jsonSql({ Pack: label })},${weightValue},${sql(weightUnit)},${sql(label)},'active',${v},${sql(now)},${sql(now)});`,
    );
    lines.push(
      `INSERT INTO variant_prices (id,variant_id,market_code,currency_code,list_amount_minor,sale_amount_minor,starts_at,tax_inclusive,status,created_at,created_by) VALUES (${sql(`vpr_catalogue_${n}_${v}`)},${sql(variantId)},'IN','INR',${priceRupees * 100},NULL,${sql(now)},1,'active',${sql(now)},${sql(actor)});`,
    );
    lines.push(
      `INSERT INTO inventory_levels (variant_id,location_id,on_hand,reserved,reorder_threshold,version,updated_at) VALUES (${sql(variantId)},'loc_mumbai',100,0,10,1,${sql(now)});`,
    );
  });
});
...
lines.push(
  "",
  "-- Replace the homepage hero, catalogue rails and farm story with the new live set.",
);
const homeSlides = [
  {
    imageUrl: "/banners/home/catalogue/01-complete-organic-range.webp",
    imageAlt: "True Grit traditional grains, pulses, seeds and cold-pressed oils",
    href: "/shop",
    label: "Explore the complete organic range",
    enabled: true,
  },
  {
    imageUrl: "/banners/home/catalogue/02-vikas-farms.webp",
    imageAlt: "Traditional grain harvest at Vikas Farms",
    href: "/farms/vikas-farms",
    label: "Meet Vikas Farms",
    enabled: true,
  },
  {
    imageUrl: "/banners/home/catalogue/03-traditional-small-batch.webp",
    imageAlt: "Traditional small-batch flour and grain processing",
    href: "/blog/from-farm-to-flour-how-true-grit-products-are-made",
    label: "See how your food is made",
    enabled: true,
  },
  {
    imageUrl: "/banners/home/catalogue/04-cook-with-true-grit.webp",
    imageAlt: "A traditional Indian meal prepared with True Grit pantry staples",
    href: "/recipes",
    label: "Cook with True Grit",
    enabled: true,
  },
];
...
lines.push(
  "",
  `-- Generated ${articleCount} substantial articles and ${recipeCount} complete recipes from all 310 supplied topics.`,
);
